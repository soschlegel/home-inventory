...
export const createContainerType = (data: { key?: string; name: string; nameDe?: string; nameEn?: string; icon?: string; color?: string }) =>
  api.post<ContainerType>('/container-types', data).then((r) => r.data);

export const updateContainerType = (
  id: string,
  data: { key?: string; name?: string; nameDe?: string; nameEn?: string; icon?: string; color?: string },
