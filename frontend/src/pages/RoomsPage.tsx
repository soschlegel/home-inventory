...
export default function RoomsPage() {
  ...
  const update = useMutation({
    mutationFn: (id: string) =>
      updateRoom(id, { name: editName, icon: editIcon || undefined }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['rooms'] });
      setEditingId(null);
    },
        ...
        <div className="mb-6 bg-white border border-indigo-400 rounded-xl p-4">
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && editName) update.mutate(editingId);
            ...
            <button
              type="button"
              onClick={() => update.mutate(editingId)}
            ...
            <button
              ...
              className="text-gray-500 px-3 py-2 rounded-lg text-sm hover:bg-gray-100 border border-gray-200"
            >
              {t('common.cancel')}
            </button>
          </div>
